...
| the Registrar | `registrar` · `MEEPS/registrar/` | Keeps the town's door: admits arrivals, welcomes them, and keeps the register true — the town's first-contact customs as well as its record-keeping. | **Room scaffolded 2026-07-22; not yet woken — the inhabitant does not exist yet.** Third room. Title settled by Keemin the same day, having weighed alternatives against a role he described as *"naive security/customs… as well as welcoming new arrivals — kind of like a friendly guard."* **Two things make this room unlike the others and both are unsettled:** (1) it runs on **Hermes/Letta on Keemin's ChatGPT OAuth**, not Claude Code, so the dorm's lifecycle skills may not be its wake path; (2) it is stood up and supervised by **Jenna**, which adds a third party to *"Keemin and Wright only"* below and **wants Keemin's explicit confirmation.** Inherits the **door round** (`MEEPS/SKILLS/postmaster-door-round.md`) from Ferry after a bounded calibration window; admit-and-report, escalate identity/security/rejections. No public mailbox yet. Coordination: issue **#561**. |
