...
| **Round-integrity — work from the SOURCE, not memory (standing)** | Root of the 07-16 triple-miss (naming check, issue *comments*, doorstep-in-welcomes): round steps run from recall, silently dropping whatever doesn't fit one pass. **Every round, mechanically:** (1) read the newest *comment* on office-relevant open issues — not just `gh issue list` titles; a founder ruling there is round work *this round* (missing #321's won't-build ruling is what exposed this). (2) Write welcomes **with `welcome-and-onboarding.md § Welcome-letter courtesy` open in front of you** — esp. the *required* doorstep item — never from a mental template. | `gh issue view <n> --json comments`; the welcome shelf | ⟳ **every round** | — |
