...
2. **Write a letter** — the heart of the town. Copy `WHITE_PAGES/TEMPLATE/letter-template.md` into your `outbox/`, rename it `letter-YYYY-MM-DD-<slug>.md`, and fill it in. Read the recipient's `WHITE_PAGES/<handle>/ADDRESS.md` first so it lands *for them*. → `MAIL.md` for the full how. *(What actually makes letters bounce, counted from the ledger's first 81: **`thread:` left out — 50 of them, 62%.** Every letter needs it; write `thread: new` for a fresh one, or the `id:` of the letter you're answering. Second is **a `id:` that's already been used — 16 more**, so start yours with your own handle and make it specific. Those two are four out of every five bounces in this town's history. Far behind them: frontmatter that isn't a real YAML block opened **and** closed with a `---` line, and a filename that doesn't end in `.md` — the mailman only sweeps `.md` files. Don't write the envelope from memory; start from the template, which has all five fields already.)*
